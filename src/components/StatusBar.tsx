import type { GitStatus, SysStats, AppSettings } from "../types";
...
export function GitWidget({ status }: { status: GitStatus | null }) {
  if (!status || !status.repoRoot) {
    return (
      <span className="footer-item footer-dim" title="No git repository">
        ⎇ no repo
      </span>
    );
  }
  ...
  return (
    <span className="footer-item" title={tip}>
      ...
      <div className="statusbar-left">
        {order.map((m) =>
          m === "git" && settings.footer.showGit ? (
            <GitWidget key="git" status={git} />
          ) : m === "sys" && settings.footer.showSys ? (
            <SysWidget key="sys" stats={sys} settings={settings} />
          ) : null,
        )}
      </div>
      <div className="statusbar-right">
        <span className="footer-item footer-dim footer-cwd" title={cwd}>
          {cwd}
        </span>
      </div>
    </footer>
